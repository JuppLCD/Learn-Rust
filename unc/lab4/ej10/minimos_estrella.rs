...
fn main() {
    ...
    // La validacion de elem_en_arreglo se realiza debido a que i32::MAX es impar (2147483647)
    if min_impar != i32::MAX || elem_en_arreglo(&arr, i32::MAX) {
...
fn elem_en_arreglo(arr: &[i32], elem: i32) -> bool {
    for elem_in_arr in arr {
        if &elem == elem_in_arr {
            return true;
        }
    }
